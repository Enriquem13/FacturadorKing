...
{
    public partial class catTipoplazos : Form
    {
        funcionesdicss funcionesgenerales = new funcionesdicss();
        public catTipoplazos()
        {
            InitializeComponent();
            cargardatos();
        }
        ...
        public void cargartiposplazos()
        {
            try {
                //cargamos los tipoplazos existentes
                conect con_plazos = new conect();
                String squery_plazos = "select * from tipoplazo " +
                                        " left join grupoplazo on tipoplazo.GrupoPlazoId = grupoplazo.GrupoPlazoId " +
                                        " left join grupo on tipoplazo.Grupoid = grupo.GrupoId;";
                MySqlDataReader resp_plazos = con_plazos.getdatareader(squery_plazos);
                ...
                while (resp_plazos.Read())
                {
                    ...
                    if (funcionesgenerales.validareader("TipoPlazoIndAct", "TipoPlazoIndAct", resp_plazos).Text == "1")
                        ...
                        dgvTioplazos.Rows.Add(int.Parse(funcionesgenerales.validareader("TipoPlazoId", "TipoPlazoId", resp_plazos).Text),
                                            funcionesgenerales.validareader("TipoPlazoDescrip", "TipoPlazoDescrip", resp_plazos).Text,
